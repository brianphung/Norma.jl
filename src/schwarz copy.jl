...
function save_stop_solutions(
    ...
    for i ∈ 1:schwarz_controller.num_domains
        ...
        if sims[i].model.inclined_support == true
            ...
            schwarz_controller.stop_disp[i] = global_transform' * sims[i].integrator.displacement
            schwarz_controller.stop_velo[i] = global_transform' * sims[i].integrator.velocity
            schwarz_controller.stop_acce[i] = global_transform' * sims[i].integrator.acceleration
        else
