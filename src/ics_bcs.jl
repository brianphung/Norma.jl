...
function SMDirichletInclined(input_mesh::ExodusDatabase, bc_params::Dict{Any,Any})
    ...
    rotation_matrix[1,1] = bc_params["normalx"]
    rotation_matrix[2,1] = bc_params["normaly"]
    rotation_matrix[3,1] = bc_params["normalz"]
    rotation_matrix[3,2] = rotation_matrix[3,2] = 0
    rotation_matrix[3,3] = 1 
    rotation_matrix[:,2] = cross(rotation_matrix[:,1], rotation_matrix[:,3])
    # for i in range(1,3)
    #     if isapprox(rotation_matrix[i,1], 0.0, atol=1e-16)
    #         bc_free_dofs[i] = true
    #     end
    # end
